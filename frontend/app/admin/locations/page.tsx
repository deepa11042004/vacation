...
export default function LocationsPage() {
  ...
  const load = useCallback(() => {
    ...
    api
      .get<{
        data: { locations: Location[]; total: number };
      }>(`/locations?${params}`)
      .then((res) => {
  ...
  async function handleSave() {
    if (!form.location_name || !form.country || !form.type) {
      setErr("Location name, country and type are required.");
      return;
    }
    setSaving(true);
    setErr("");
              ...
              {locations.map((l) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`text-xs font-bold px-2 py-0.5 rounded-full ${l.type === "DOMESTIC" ? "bg-blue-50 text-blue-700" : "bg-purple-50 text-purple-700"}`}
                    >
                      {l.type}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <span
                      className={`text-xs font-bold px-2 py-0.5 rounded-full ${l.status === "ACTIVE" ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}
                    >
                      {l.status}
                    </span>
                  </td>
